		...
		for (int i = 0; i < count; i++) {
			...
			switch (attr) {
				...
				case R.styleable.chartView_xylinewidth://xy坐标轴宽度
					xylinewidth = (int) array.getDimension(attr, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, xylinewidth, getResources()
                            .getDisplayMetrics()));
					break;
				...
				case R.styleable.chartView_xytextsize://xy坐标轴文字大小
					xytextsize = (int) array.getDimension(attr, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, xytextsize, getResources()
                            .getDisplayMetrics()));
					break;
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		if (changed) {
			...
			for (int i = 0; i < yValue.size(); i++) {//求取y轴文本最大的宽度
				...
				if (temp > textYWdith) textYWdith = temp;
			}
			int dp2 = dpToPx(2);
			int dp3 = dpToPx(3);
...
//            //X轴文本最大高度
			...
			for (int i = 0; i < xValue.size(); i++) {//求取x轴文本最大的高度
				...
				if (rect.height() > textXHeight) textXHeight = rect.height();
				if (rect.width() > xValueRect.width()) xValueRect = rect;
			}
	...
	private void drawXY(Canvas canvas) {
		...
		if (xLength < width) xLength = width;
		...
		for (int i = 0; i < xValue.size(); i++) {
			...
			if (x >= xOri) {//只绘制从原点开始的区域
				...
				if (i == selectIndex - 1) {
					...
					canvas.drawRoundRect(x - xValueRect.width() / 2 - dpToPx(3), yOri + xylinewidth + dpToPx(1), x + xValueRect.width() / 2 + dpToPx(3), yOri
                            + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2), dpToPx(2), dpToPx(2), xyTextPaint);
				} else {
					canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
				}
			}
		...
		animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator valueAnimator) {
				...
				if (velocity < 0 && xInit > minXInit) {//向左滑动
					if (xInit - value <= minXInit) xInit = minXInit;
					else xInit = xInit - value;
				} else if (velocity > 0 && xInit < maxXInit) {//向右滑动
					if (xInit + value >= maxXInit) xInit = maxXInit;
					else xInit = xInit + value;
				}
				invalidate();
			}
	...
	private void clickAction(MotionEvent event) {
		...
		for (int i = 0; i < xValue.size(); i++) {
			...
			if (eventX >= x - rect.width() / 2 - dp8 && eventX <= x + rect.width() + dp8 / 2 && eventY >= y - dp8 && eventY <= y + rect.height() + dp8 &&
                    selectIndex != i + 1) {
				selectIndex = i + 1;
				invalidate();
				return;
