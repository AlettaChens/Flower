...
import static com.example.wordchen.web.Constant.BASEURL;
import static com.example.wordchen.web.Constant.WEBDIR;
	...
	public static RxHttpRequestHandler getInstance(Context context) {
		...
		if (retrofit == null) {
			synchronized (WebAPIManager.class) {
				if (retrofit == null) {
					retrofit = new Retrofit.Builder().baseUrl(BASEURL).client(InitOkHttp(context, WEBDIR)).addConverterFactory(GsonConverterFactory.create())
